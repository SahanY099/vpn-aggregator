...
export const excludedType: string[] = ["https", "http", "ssr", "ss", "vless"];
...
class Main {
  ...
  async scrape() {
    const unfilteredConfigUrls = (await scraper.start(this.urls)).result;
    logger.log(LogLevel.info, `Total Found (Filtered) -> ${unfilteredConfigUrls.length}`);
    
    const configUrls = unfilteredConfigUrls.filter(str => pattern.test(str));
    this.configUrls.push(...configUrls);
    
    console.log("");
    logger.log(LogLevel.info, `Total Found (Filtered) -> ${this.configUrls.length}`);
  }

  async connectTest() {
    for (let i = 1; i <= this.numberOfTest; i++) {
      const unfilteredConfigUrls = structuredClone(this.configUrls);
      const configUrls = unfilteredConfigUrls.filter(str => pattern.test(str));
